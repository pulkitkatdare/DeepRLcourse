...
def main():
	...
	parser.add_argument('--training', type=int, default=5,
						help='Number of expert roll outs')
	...
	for _ in tqdm(range(args.training)):
		...
		for steps in range(args.num_rollouts):
			...
			while not done:
				...
				rew += r
			tqdm.write(" iter: %3d, episode steps: %3d, rewards: %f" %(steps, j, rew))
		observations = np.array(observations)
		states = np.concatenate((states, observations), axis = 0)
		new_exp_len = observations.shape[0]
		expert_expected_actions = []
		with tf.Session():
			for k in range(0,new_exp_len,args.batchsize) :
				expert_expected_actions.append(policy_fn(observations[k:k+args.batchsize]))
			action = np.concatenate([actions]+expert_expected_actions,
		                                             axis=0)
	mean_reward = 0
	OUTPUT_RESULTS_DIR = './saver'
	ENVIRONMENT = args.envname
	TIMESTAMP = datetime.now().strftime("%Y%m%d-%H%M%S")
	SUMMARY_DIR = os.path.join(OUTPUT_RESULTS_DIR,'dagger', ENVIRONMENT, TIMESTAMP)	
